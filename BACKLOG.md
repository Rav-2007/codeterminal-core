...
| What bugs are open? | [`docs/OPEN_ITEMS.md`](docs/OPEN_ITEMS.md) — read the Status column; **open: 24, 34, 35, 36, 37** — the security residuals carried over from [`AGENT_SECURITY_AND_CAPABILITY_AUDIT.md`](AGENT_SECURITY_AND_CAPABILITY_AUDIT.md), which had been tracked by git and referenced by nothing. This cell said *none* while that report listed six open findings, and it was not lying — the register it is checked against had never heard of them. Enforced by `scripts/docs-claims.sh`, which fails the build when this cell and that Status column disagree — in either direction. |
