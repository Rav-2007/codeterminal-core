...
func TestEditsUndo_ModifiedFileIsGuardedNotClobbered(t *testing.T) {
	...
	var undoOut bytes.Buffer
	if err := runUndoSession(root, sessionDir, false, strings.NewReader("n\n"), &undoOut, discardLogger()); err != nil {
		t.Fatalf("runUndoSession: %v", err)
	}
	...
	if err := runUndoSession(root, sessionDir, true, strings.NewReader(""), &forcedOut, discardLogger()); err != nil {
