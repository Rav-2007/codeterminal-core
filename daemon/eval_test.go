...
func buildRealHelperBinary(t *testing.T) string {
	t.Helper()
	binPath := filepath.Join(t.TempDir(), "codeterminal-embedder-helper")
...
func TestEvalRetrievalQuality(t *testing.T) {
	...
	scan, err := buildIndex(ctx, evalRoot, embedder, store)
	if err != nil {
	...
	for _, q := range queries {
		hits, err := retrieveTopK(ctx, q.Query, 3, embedder, store)
		if err != nil {
