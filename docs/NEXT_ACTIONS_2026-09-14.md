...
2. **Decision-memo item 3 — inbound model text not redacted (= R1.6). ACCEPTED IN WRITING**,
   ...
   fixed. `(R)`
3. **`.codeterminal/backups/` — the cap and the sweep BOTH EXIST.** The seed row's `(U) no cap
...
6. **I ran exactly one test.** `go test -count=1 -run
   ...
   `ok codeterminal/daemon 0.067s`. I did **not** run `make check`, the fuzz suite, the race
