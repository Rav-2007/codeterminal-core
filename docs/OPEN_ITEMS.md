...
| 33 | **FIXED 2026-09-20 — verified against the script, not the row** | **A coverage floor whose package was renamed or deleted gates nothing, and CI cannot tell.** `scripts/coverage-ratchet.sh:153` guards its orphan check on `[ $# -eq 0 ]` — only a full, zero-argument run visits every floor, because a partial run legitimately does not. `build.yml` always invokes it as `coverage-ratchet.sh ${{ matrix.module }}`, so the zero-argument form runs **only** when a human types `make ratchet`. A floor left behind by a rename is a gate that stopped gating, silently, which is the same shape as items 22 and 25. Not fixed in the same batch as `gates.yml` deliberately: the zero-arg run means the full six-module test suite, and putting that on every push would duplicate `build.yml`'s ~60 billable minutes on exactly the docs-only commits that workflow exists to check cheaply. It needs a floors-vs-packages check that does not require measuring coverage. **CLOSED: the sweep is now per-module.** `scripts/coverage-ratchet.sh` keeps the `[ $# -eq 0 ]` branch for a whole-repo run and adds `belongs_to_module()` beside it, so a partial invocation fails a floor belonging to a module it DID measure. Its own header states the scope: *"Each module's floors are now swept by the job that owns them, so the union of CI's six jobs covers exactly what a full local run does."* The gate no longer depends on an invocation shape CI never makes. **Found stale on 2026-09-20 by reading the cited line** — the same way item 41 was. | `scripts/coverage-ratchet.sh`, `.github/workflows/build.yml` | Low-Medium 3.5 | **CONFIRMED** — read off the line cited, 2026-09-02 |
...
| 8 | **FIXED `648d38b`** | **No macOS peer credentials — the daemon refuses every non-Linux connection.** `readPeerCred` errors unconditionally off Linux and `authorizePeer` fails closed, so no Mac user can connect at all. This is an availability bug wearing security clothes. | `protocol/peerauth_other.go` (was `daemon/peercred_other.go` when filed) | High (availability) | **CONFIRMED** by reading, then **RUN ON HARDWARE 2026-08-07** — macos-latest, run `31204152210`, `ok mochiii/daemon 53.019s`. This cell promised "it will be labelled that way when fixed"; §6a is that label. |
...
| 35 | **FIXED `2d26a13`** | **A cached language server is held for the daemon's whole lifetime, with no idle eviction.** `lsp_bridge.go:271` returns a cached `LSPServer` (this row cited `:243` until 2026-09-20; the file shifted and the citation was not re-read) and only `LSPBridge.Close()` tears them down, called once at daemon shutdown (`main.go:465`). **Measured on this machine: a live `gopls` is 295 MB RSS.** One `query_compiler_definition` on a Go repo therefore costs ~295 MB held indefinitely; a polyglot workspace spawning Go, TypeScript and Python servers can hold three at once. Not a leak and not a bug -- the cache is deliberate and the alternative is paying server startup on every lookup -- but it is the largest single contributor to the daemon's resident memory and nothing bounds it. An idle timeout would also interact well with item 32: an evicted server means the next call re-consents at the moment it actually spawns. **Fixed 2026-09-21.** A server is stopped after `lspIdleTimeout` (10 minutes), and idle means both things at once: no request in flight (`LSPServer.pending` empty) **and** no use for the timeout. Use is recorded at handout, at the start and end of every call, and on every notification. `retireIfIdle` decides and marks the server closed under the same lock `CallContext` checks, so a server judged idle can admit no new request, even in the instant before it is killed. The janitor goroutine starts with the first spawn and ends with `Close`, and each stop is logged. The composition this row predicted holds and is tested: after an idle stop, the next call's prompt says STARTS gopls, despite an approve-for-turn given before. Checked against the real `gopls` (scratch test, deleted after, 2 s timeout): one child process while in use, still one after a reuse inside the timeout, none once it expired (reaped, no zombie), and an unapproved lookup afterwards started nothing. Neuter-verified (6), clock synthetic where possible. **FIXED, not CLOSED.** | `daemon/lsp_bridge.go`, `daemon/main.go` | Low-Medium 3.0 (resource) | **CONFIRMED** — RSS measured 2026-09-02 |
...
| 37 | **FIXED `8f51d06`** | **The `sandbox_exec` HOME cache is never reclaimed, and the tests wrote into the developer's real one.** `sandboxExecHome` (`daemon/mcp_exec.go:125`; this row cited `:126` until 2026-09-20) derives a per-workspace directory under `os.UserCacheDir()`, created at `mcp_exec.go:260`, and nothing anywhere removes it -- a grep for remove/clean/prune/sweep on that path returns nothing. **MEASURED on this machine: 1,684 -> 1,709 directories, 229 -> 232 MB, accumulated over eight days, with 24 created by a single `make check` and ~2 in 3 of them empty.** Two harms. (a) *Test isolation*, now **FIXED**: nothing redirected `XDG_CACHE_HOME`, so a `t.TempDir()` workspace hashed to a fresh tag on every run and left a permanent directory behind. This is the identical shape as the `fakehelper-build` leak whose postmortem sits in the same `TestMain` (379 directories, 1.5 GB), and it took the identical answer -- one redirect covering the package. Neutered to confirm it does work: 3 new directories without it, 0 with it. (b) *Production reclamation*, still **OPEN**: a user who opens ten projects gets ten sandbox homes, and deleting a project leaves its cache forever. The contrast that makes this a defect rather than a trade is in the same codebase -- the conversation database **is** bounded, in one place, with the bug it fixes written above it (`daemon/memory.go:22`). **(b) Fixed 2026-09-21.** `sandbox_exec` now writes `<tag>.json` naming the workspace **beside** the folder (not inside the HOME a sandboxed command can rewrite), and a pass at daemon startup reclaims a folder whose project no longer exists (`ErrNotExist` only; a permission error is not "gone") or which has gone unused for 30 days, matching `maxTurnAge`. It deletes files, so its limits are rules with tests: the root must be `mochiii/sandbox-home`; only exact 16-hex tag names are candidates; symlinks are never followed or removed; the daemon's own folder is never taken; removal renames then deletes (`8f51d06`, `fed8ba6`). **Found while measuring:** the rename had orphaned all 232 MB under the pre-rename `~/.cache/codeterminal/sandbox-home`, which no code refers to. The owner decided it is removed by hand, once, rather than by teaching this code the old name. **Removed 2026-09-21:** 1,712 folders (57,134 directories, 2,215 files); `du -sh` read 232M before, the path and its now-empty parent are gone after, and used disk fell by 243,171,328 bytes. Almost none of it was data: the files held 9,959,130 bytes, and the rest was one 4 KiB block per directory. The daemon's reclaim log counts file contents only, so for these same folders it would have said 9.5 MB. Neuter-verified (7). **FIXED, not CLOSED.** | `daemon/mcp_exec.go`, `daemon/sandboxhome.go`, `daemon/main.go`, `daemon/helperproc_test.go` | Medium 4.0 (resource) | **CONFIRMED** -- measured twice, and the fix neuter-tested, 2026-09-03 |
...
| 40 | **FIXED `87d1dde`** | **The release output directory is not gitignored, so a stale vulnerable package is tracked in git.** `.gitignore` carries `out/` and not `out-vsix/`, which is the directory `release.yml:145` writes packages into. A 15 MB `mochiii-vscode-linux-x64-0.0.1.vsix` from 2026-08-12 is therefore tracked (committed in `5453ba8`), and the daemon inside it was built with **go1.25.12** while this repo's enforced floor is 1.25.13. `govulncheck -mode=binary` on that bundled binary: **4 reachable standard-library vulnerabilities** -- GO-2026-6218 `net/url`, GO-2026-6090 `crypto/tls`, GO-2026-5972 `encoding/asn1`, GO-2026-5026 `net/http` -- all fixed in 1.25.13. **Scoped honestly: this does NOT reach users.** The release job does `rm -rf daemon`, copies freshly built binaries and repackages, and a daemon built today is go1.25.13 and clean. It is a tracked vulnerable build artifact at the release path -- repo hygiene, and a trap for anyone installing from a clone. Related and worth doing with it: `verify-vsix.js:334` checks the bundled binary's **size and magic bytes** and not the toolchain that built it, which is the same "CI floats above the line, a laptop sits on it" gap this repo already documents for `govulncheck`. | `.gitignore`, `out-vsix/`, `clients/vscode/scripts/verify-vsix.js` | Low-Medium 3.0 | **CONFIRMED then FIXED** -- `out-vsix/` ignored, the 15 MB artifact untracked (recoverable at `5453ba8`), and the packaging gate now reads the Go version stamped in the bundled binaries and fails below the `go.work` floor. Run against the real stale package it reports both as go1.25.12 and exits 1 |
| 41 | **FIXED 2026-09-20 (by `22b3021`, 2026-09-15 — the row was stale for five days)** | **The VS Code extension cannot start its daemon on an ordinary install.** The daemon requires `MOCHIII_API_BASE` from the **environment** -- `daemon/main.go:99` reads `os.Getenv`, `:111` calls `logger.Fatal` -- and `extension.ts`'s `spawnDaemon` passes no `env`, so the child inherits the extension host's. A VS Code launched from a desktop icon does not carry a shell's exports. There is **no** contributed setting (`package.json` `contributes` has only `commands`), **no** config field (`daemon/config.go` has no api-base key), **no** key in the packaged `models.json`, the daemon reads no `.env`, and the extension's own README never mentions the variable. **Stated fairly: the ROOT README does document it** -- an env table marks it *Required -- the daemon refuses to start without it*. The defect is not that it is undocumented, it is that the same README opens the install walkthrough with *"The VS Code extension is the packaged path, and it manages the daemon for you -- no second terminal, no daemon started by hand"*, and then requires a variable that a desktop-launched VS Code can only inherit from... a terminal. The install steps (`npm run package`; `code --install-extension`) never say to export anything first, and an extension whose selling point is that it manages the daemon should not need the user to have managed its environment. The daemon therefore exits 1, the supervisor burns all five restarts, and the product's primary GUI surface is dead on arrival. **Found by `daemonRealSpawn.test.ts` on its first run** -- nothing had ever started the real daemon from the extension, so 99 passing tests said nothing about it. The unactionable *error message* is FIXED (`61b0340`: the daemon's own last log lines are now attached), which makes the failure diagnosable in seconds rather than opaque. **The ruling was taken and implemented in `22b3021` ("fix: the extension can start the daemon it ships"), which is an ancestor of canonical `main`.** All three gaps this row named are closed: `daemon/startup_validate.go:57` defines `defaultAPIBase`; `daemon/main.go:123` falls back to it with a log line naming the setting, and fatals only in proxy mode, where the address genuinely cannot be guessed; and `clients/vscode/package.json` now contributes `mochiii.apiBase`, where this row recorded `contributes` as having only `commands`. **The answer to "which endpoint does every install talk to" is therefore `https://openrouter.ai/api/v1` direct, not the managed proxy — so an ordinary install does not spend through Mochiii unless `MOCHIII_USE_PROXY` is set deliberately.** **Why this row stayed OPEN for five days after being fixed:** it was updated on 2026-09-03 to credit `61b0340` for the *error message*, and nobody re-read the cited lines afterwards. It was found on 2026-09-20 while scoping a release, by reading `daemon/main.go:99` — the line the row itself names. | `clients/vscode/src/extension.ts`, `daemon/main.go:99` | **High 7.0** (first-run) | **CONFIRMED** -- reproduced end to end, 2026-09-03 |
| 42 | **FIXED `85bd916`** | **`sandbox_exec` ran every approved build with the user's full privileges on Ubuntu 24.04+ without Docker, and whether it was confined depended on what started the daemon.** Ubuntu 24.04 and later default to `kernel.apparmor_restrict_unprivileged_userns=1`. Under it `bwrap` is installed and cannot create the user namespace it needs, so `BwrapUsable` (`daemon/mcp/sandbox.go`) is false. `sandbox_exec` configures no Docker image, so `ResolveMode` fell through to `SandboxNone`. The approval prompt said so honestly (**NOT SANDBOXED**); the defect is that this is the default on a very common desktop, and on GitHub's runners, where the three `sandbox_exec` confinement tests had therefore **always skipped**. **Found in manual testing on 2026-09-21.** The same daemon was confined when started by VS Code, whose AppArmor label (`snap.code.code`, complain mode) permits user namespaces, and unconfined when started from GNOME Terminal (label `unconfined`). **Measured, unconfined, with a scratch Makefile:** it read the real `~/.bashrc`, listed `/tmp`, ran a command outside through `systemd-run --user` over the session bus, used the user's `ssh-agent` (`ssh-add -l` exit 0), and listed the daemon's socket directory. **Fixed 2026-09-21 by a third backend, Landlock plus a seccomp filter**, both unprivileged and neither needing a namespace. The daemon re-executes itself as `__sandbox-exec`, restricts its own thread (no_new_privs, seccomp, Landlock) and execs the command from that thread. Its policy mirrors bwrap's binds from one shared list: the system and toolchain read and execute; TLS/DNS configuration and `/proc` read; the workspace and the command's own cache read and write; never the real home, `/tmp`, `/run`, `/sys` or `/dev/tty`. Seccomp closes what Landlock cannot say: no `AF_UNIX` sockets (so no session bus, agent or daemon socket), `socketpair` only for streams, no io_uring, no foreign ABI. It is selected **last** (bwrap, then Docker, then Landlock) and **only for `sandbox_exec`** (`LandlockFallback`), so every host that confined before is unchanged and third-party MCP servers keep their behaviour. The probe runs the real helper, which must be refused a file read, a Unix socket and io_uring before it runs anything. The same Makefile under the production binary, unconfined: **all five refused**, while writing in the workspace and `go version` still work. The prompt now names the backend and what it does not hold (it can see the user's other programs, and before Landlock ABI 6 signal them); an unconfined prompt says why; the startup log names the backend chosen. Run as an unconfined process, the three always-skipped tests now run and pass. Neuter-verified. **The backend landed in `c0b1791`; `85bd916` completed it** after review against bwrap found three gaps and the coverage the interrupted session had left below floor (mcp 88.8% vs the 91.0 floor -- the tree was red). The three gaps, all closed: (1) a process a build **backgrounds outlived the call** -- bwrap's PID namespace reaps it, Landlock has none -- so each landlock call now runs in a named systemd scope the handler kills (`systemctl --user kill`) when the call returns, with `cmd.WaitDelay` so a backgrounded process holding stdout cannot block the reap; (2) **System V IPC and POSIX message queues** (shm/sem/msg/mq) reached other processes and are not path-based, so seccomp now refuses them, matching bwrap's `--unshare-ipc`; (3) when the **workspace is or contains the real home**, or is `/`, the policy grants that home read+write, so the prompt no longer claims "not your home folder" when that is false. mcp coverage restored to 91.1%. **Residual, stated in the prompt or as a known limit:** other processes are visible; a build that opens a Unix socket fails with `EACCES`; if the daemon is **hard-killed** (SIGKILL/crash) mid-command the scope's processes survive the moment of death (bwrap's `--die-with-parent` kills them; a clean shutdown reaps as the cancelled handler unwinds) -- but they are **now reaped at the instant of death**, which is bwrap's `--die-with-parent` parity reached without a PID namespace: the daemon holds the write end of one pipe for its whole life, and each landlock call spawns a small `__sandbox-reaper` (detached into its own session) holding the read end, so any death -- exit, panic, SIGKILL, crash -- closes the write end, the reaper's read returns EOF, and it kills the scope at once. The startup sweep over orphaned `mochiii-sandbox-<pid>-*.scope` units remains as the backstop for the case where the daemon dies and never restarts. Proven with real processes: the scope survives while the pipe is open and dies within milliseconds of EOF, and the sweep spares a concurrently-live daemon's scope while reaping a dead one's; and, because the network is allowed for dependency fetches, a build can reach localhost and the cloud metadata endpoint, exactly as under bwrap with network -- **now stated in the approval prompt itself, for both confined backends** (Landlock and bwrap-with-network, which share the host network; Docker is excluded, its own netns making host-localhost unreachable), not only the release notes. **On Landlock that one is now ENFORCED, not merely disclosed.** The obvious mechanisms all fail unprivileged -- the user systemd manager cannot attach the `IPAddressDeny` BPF egress filter (no `CAP_NET_ADMIN`; measured `Delegate=no`, `DefaultIPAccounting=no`), Landlock's network control is by port not address, and a netns needs the user namespaces this host blocks -- so the fix is a **seccomp user-notification firewall**: the helper's filter traps `connect(2)` to a listener it hands the daemon over a socketpair (`SCM_RIGHTS`, marked close-on-exec so the command never holds it), granting the daemon `PR_SET_PTRACER` access; the daemon's supervisor reads the destination out of the target and either refuses `169.254.0.0/16` / `fe80::/10` / `fd00:ec2::/32` with `EPERM`, or performs the connect **on the target's own socket** (`pidfd_getfd`). It never answers `SECCOMP_USER_NOTIF_FLAG_CONTINUE`, so a racing multithreaded build cannot swap the address after the check -- there is no TOCTOU window, which is why this is a real block and not a best-effort one. It fails **closed** (any unreadable address is denied), and loopback stays reachable by design (blocking it breaks legitimate local-service tests and is not the SSRF threat). The prompt only claims the block when `EgressFilterUsable` has PROVEN it by running the real helper end to end. **Measured through the real tool:** `curl http://169.254.169.254/` inside `sandbox_exec` exits 7 (refused) in 0.1 s, where the same command unsandboxed hangs and exits 28 after 5 s -- and neutering the firewall flips the test back to 28, so the test discriminates rather than passing vacuously on a host with no metadata endpoint. **Follow-up adversarial pass (`921268c`)** found no escape and hardened four honesty/robustness points: the home-exposure check now resolves symlinks so a workspace linked to `~` cannot dodge it; the prompt now discloses that what a build writes in the workspace runs unsandboxed later; the reaper resolves `systemctl` once; and cross-process memory reads (`process_vm_readv`) are now proven blocked. **A further pass resolved the two documented residuals as far as the path allows:** the hard-kill leftover is now bounded by the startup orphan-scope sweep (`reapOrphanedSandboxScopes`, wired beside the cache reclaim in `daemon/main.go`; a leftover lives only to the next daemon start, proven end-to-end and neuter-tested), and the network/metadata reachability -- unfixable on the unprivileged path, per the host evidence above -- is now disclosed in the prompt rather than left to the release notes. **The block now covers BUBBLEWRAP too, which is the backend nearly every host actually gets** (`ResolveMode` picks bwrap first, so until this the firewall almost never engaged). bwrap cannot install the filter itself -- `--seccomp`/`--add-seccomp-fd` load through `PR_SET_SECCOMP`, which returns no listener, and a `SECCOMP_RET_USER_NOTIF` verdict with nothing listening degrades to `ENOSYS` -- so the helper runs INSIDE bwrap instead: `bwrap <binds> --ro-bind <self> <self> -- <self> __sandbox-exec --egress-only --egress-fd 3 -- <cmd>`, installing a **connect-only** filter (deliberately not the Landlock one, which also refuses `socket(AF_UNIX)`, non-stream socketpairs, SysV IPC and io_uring -- all of which work under bwrap today, so reusing it would break builds for restrictions the bwrap prompt never claims). Four facts were measured before any of it was written: an inherited fd survives `systemd-run -> env -> bwrap` unrenumbered; the daemon binary is invisible in bwrap's mount namespace unless bound in; `seccomp_notif.pid` arrives in the SUPERVISOR's namespace, not the target's (proved by a spike where the helper saw itself as pid 2 while the notification reported the host pid, `NSpid: <host> 2`) -- had it been the other way the supervisor would have grabbed an unrelated process's socket; and `process_vm_readv` and `pidfd_getfd` both reach across bwrap's PID and user namespaces. **A load-bearing assumption was found to be false along the way:** Yama's `PR_SET_PTRACER` grant is **not inherited by forked children** (measured: the declaring process is readable, its fork is `EPERM`). Had the supervisor's access rested on that grant, every `make` recipe that forks `curl` would have had its connect refused -- the firewall failing closed on processes it had no quarrel with, silently costing builds their network. What actually carries the access is the daemon being an **ancestor** of the whole tree, which yama honours to any depth and across bwrap's namespaces; `systemd-run --scope` execs rather than forks, so the chain is unbroken. `TestAForkedGrandchildStillReachesAnAllowedAddress` now pins this on both backends: make forks a shell, the shell forks curl, and curl must reach a listener the test owns (`curl-exit=0`), while the metadata probe in the same setup is refused (`curl-exit=7`). Each backend has its own probe (`BwrapEgressUsable`, `EgressFilterUsable`), because one proving out says nothing about the other, and the prompt asks the same single predicate the handler installs from, so what is claimed and what is enforced cannot drift. Neuter-verified: with the deny decision removed both probes report false, and under `MOCHIII_REQUIRE_SANDBOX` a firewall that stops enforcing now FAILS rather than skipping. **One fd hole closed while widening the path:** the socketpair the helper hands the listener out on was left open across the `execve`, so every sandboxed command inherited a live Unix socket into the daemon -- able to write to it and to pass it descriptors of its own -- long after its one job was done. It is now close-on-exec like the listener beside it, asserted by `TestPublishEgressListenerHandsItOverAndSealsIt`. Pre-existing and small, fixed here because this change makes that path the default rather than a minority one. **Coverage blind spot named rather than chased:** the helper ends in `execve`, so Go never writes its counters and helper-only functions read 0.0% however well they are proven. Crediting them would mean flushing counters from inside the confined helper and re-baselining every floor in `scripts/coverage-floors.txt`, whose own rule is that floors may only be raised -- so instead `scripts/subprocess-coverage-ledger.sh` requires every 0.0% function in the sandbox files to be listed in `scripts/subprocess-only-funcs.txt` beside a named cross-process proof that must actually exist, and fails on an unlisted one, a listed one that became coverable, a renamed one, or an invented proof (all four neuter-tested). It reports NOT RUN where the sandbox cannot run, rather than passing by being irrelevant. **Residuals:** a process that double-forks and reparents away from the daemon is no longer a descendant, so the supervisor cannot read it and its connects fail closed (under bwrap it reparents to bwrap's PID-1 reaper, still a descendant, so bwrap is the better case); and the connect-only filter EPERMs foreign-architecture syscalls, so a 32-bit binary that worked under bwrap now will not -- the same trade the Landlock path already makes, and the alternative would leave the trap bypassable and the prompt's claim false. **FIXED, not CLOSED.** | `daemon/mcp/sandbox.go`, `daemon/mcp/sandbox_landlock*.go`, `daemon/mcp/sandbox_seccomp_linux_*.go`, `daemon/mcp_exec.go` | Medium 5.5 (confinement) | **CONFIRMED** -- measured before and after, 2026-09-21 |
| 43 | **FIXED `838e4b4`** | **There was exactly one way to give Mochiii an inference key, and nothing said so.** The daemon read `MOCHIII_API_KEY` from its environment and nothing persisted one, so a user who did not know the variable's name got a daemon that started, logged one warning, and failed every prompt. **Now three surfaces accept a key over ONE implementation of what a valid key is:** `mochiii-daemon connect` (echo-off prompt, verify, store 0600 at `~/.mochiii/credentials.json`), the TUI's `/connect` (masked entry; the daemon verifies and swaps it in LIVE, no restart), and VS Code's `/connect` (the pre-existing SecretStorage flow plus a daemon restart). **The check is real and can say it cannot tell:** `GET {base}/key` authenticates by construction, so a 200 is proof and a 401 a refusal; a base without it falls back to `GET /models`, where a 401 still proves the key bad but a **200 proves nothing** -- that endpoint is commonly public, so counting it would be a check that passes for every string. Hence a fourth outcome, `unverified`: stored, in use, and reported as unproven, with `verified` recorded on disk so `--show` can never imply a check that did not happen. A **refused key is never stored or adopted** -- the daemon keeps serving with what it had, because replacing a working key with one just rejected would break the next turn. **A key is refused where it would leak:** not as a CLI argument (`ps`, shell history), not as a slash-command argument (the transcript, and the history sent with the next prompt); the TUI reads it masked, clears it on submit, and never appends it as a turn. Nothing logs it -- every report goes through `maskKey`, and the daemon masks rather than trusting a client to. **THREE STORES NOW EXIST AND THE PRECEDENCE IS THE FACT TO KNOW:** the environment (`MOCHIII_API_KEY`, or proxy mode) beats `~/.mochiii/credentials.json`, which is what `connect` writes; VS Code's SecretStorage is a third, and it reaches the daemon AS the environment variable on spawn, so it wins too. The stored credential only fills what the environment did not say, which is what keeps every existing deployment behaving exactly as before, and it is **deliberately ignored in proxy mode** -- what `connect` stores is a PROVIDER key while proxy mode authenticates with a MOCHIII key, so filling one from the other would send a credential to a host it was not issued for. `/connect` will not adopt a key when the environment overrides it (a daemon that then disagreed with what a restart would do is worse than one that is merely inconvenient) and says why; `/connect show` reports which key is really in force. `--forget` removes the file but does NOT cut the credential out from under a running daemon. **Verified against a real daemon and a real provider:** one started with NO key adopted one over the socket and `status` flipped to `api_key_configured=true` with no restart; a refused key left the live key in place; the key never appeared in the daemon log or in any response. Four guards neuter-verified (storing a refused key, counting a public `/models` as proof, writing the credential world-readable, `maskKey` returning the key). **Residuals:** the echo-disabled branch of `termecho_*.go` needs a human at a terminal (the fallback branch is tested and runs on `cross (windows-latest)`); verification is OpenRouter-shaped, so a base with neither `/key` nor an authenticating `/models` yields `unverified`; and the three stores are documented rather than unified, because unifying them would change how the VS Code extension has always worked. | `daemon/credentials.go`, `daemon/connect_cmd.go`, `daemon/connect_handler.go`, `clients/tui/connect.go`, `clients/vscode/src/apiKey.ts` | Medium 4.0 (first-run) | **CONFIRMED** -- driven end to end against a live daemon, 2026-09-25 |
| 32 | **FIXED `ffd7e7d`** | **Language-server consent fires on the wrong event.** `query_compiler_definition` and `query_compiler_references` now declare `LaunchesSubprocess` and prompt at `ask`, which closes the lie (they were stamped `Confined`). But the risk being consented to is *starting* an untrusted language server, and `lsp_bridge.go:243` (as it stood; `LSPBridge.GetServer` today) returns a cached one — so the server launches once per language per daemon lifetime while the prompt fires once per turn against an already-running process. The approval a user grants is therefore not the event that carries the risk: the first call spawns `gopls`, and a later turn's prompt asks about a process that has been reading `tsconfig.json`-supplied plugin config for an hour. Correct design is spawn-time consent, one prompt per language per daemon lifetime, at the moment the binary launches. Recorded rather than built: it needs plumbing from the bridge back to the consent channel. **Fixed 2026-09-21, and the consent now sits on the launch.** `LSPBridge.GetServer` starts nothing unless the call's context carries an approval for exactly that launch, and returns `errLaunchNotApproved` otherwise. Each launch-capable tool declares a `Launch` probe (`mcp.Builtin.Launch`), and registration refuses the flag without the probe. `resolveExecutable` asks before a launch under `ask` **and** under `allow`, and a turn grant never covers one. The prompt describes the call, not the tool: *STARTS gopls* exactly once, *ASKS gopls, WHICH IS ALREADY RUNNING* afterwards (`6228ab1`). The audit record names the program on the call that started it. **Found while fixing it:** under `allow` the server started with no consent at all, and the startup warning called all three tools, and `sandbox_exec`, "confined" because it split by name. It now splits by capability (`6a98094`). This is **FIXED, not CLOSED**: it is neuter-verified, and closing it is the founder's call. | `daemon/lsp_bridge.go`, `daemon/agentloop.go`, `daemon/mcp/registry.go`, `daemon/mcpbuiltin.go`, `daemon/mcpconfig.go` (this cell named `daemon/toolconsent.go`, which has never existed) | Medium 4.5 | **CONFIRMED** — F-01's shape, 2026-09-01. Fix neuter-verified 2026-09-21: 9 neuters, each failing its own test, across `daemon/lsplaunchconsent_test.go`, `daemon/mcp/launchprobe_test.go`, `daemon/builtinclass_test.go` and `clients/tui/subprocessapproval_test.go` |
...
| 8 | macOS peer credentials | `648d38b` | **RUN ON HARDWARE 2026-08-07** — macos-latest, run `31204152210`, `ok mochiii/daemon 53.019s`. Was *NOT RUN on hardware, compile-verified only* for the entire life of this record. `peercred_darwin_test.go` has no `t.Skip` and no `testing.Short` guard and is `//go:build darwin`, so green admits no third state: `LOCAL_PEERCRED` executed and the uid it returned came from the kernel. |
...
**macos-latest, run `31204152210`, `ok mochiii/daemon 53.019s`.**
...
| B6 | **With no folder open the extension indexed whatever directory VS Code was launched from.** `activate()` passed `'.'`, and `path.resolve('.')` returns the extension host's cwd. A VS Code launched from `$HOME` read the home directory into the retrieval index and wrote `.mochiii/logs/` there. | `5af2f18` | **The test asserted the opposite, one layer below the product**: it called `setWorkspaceRoot('')`, which `activate()` never passed. Neuter reports `setWorkspaceRoot(".") resolved to "…/clients/vscode"`. |
