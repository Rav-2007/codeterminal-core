...
#   MOCHIII_PROXY_BASE=http://localhost:8080/v1 ./run-proxy.sh  # local proxy
...
# MOCHIII_PROXY_BASE (see below), never via MOCHIII_API_BASE.
readonly DEFAULT_PROXY_BASE="https://codeterminal-core-production.up.railway.app/v1"

readonly DAEMON="./daemon/mochiii-daemon"
...
# MOCHIII_PROXY_BASE is the ONE way to aim this script at a different
...
#
# It exists as a separate variable because MOCHIII_API_BASE cannot do this
# job safely. This script used to treat a caller-supplied MOCHIII_API_BASE
...
# "Run it" is `set -a && source .env && set +a`, and .env's MOCHIII_API_BASE
...
caller_proxy_base="${MOCHIII_PROXY_BASE:-}"
...
if [[ -f .env ]]; then
	...
	die ".env not found in $(pwd)" \
		...
		"  cp .env.example .env" \
		"  \$EDITOR .env      # set MOCHIII_PROXY_KEY=mochi_..."
fi

# Resolve which proxy to use: an explicitly exported MOCHIII_PROXY_BASE
...
elif [[ -n "${MOCHIII_PROXY_BASE:-}" ]]; then
	proxy_base="${MOCHIII_PROXY_BASE}"
else
...
# MOCHIII_PROXY_BASE to it deliberately — but that is still a credential
...
if [[ "$proxy_base" == *"openrouter.ai"* ]]; then
	die "MOCHIII_PROXY_BASE points at OpenRouter ($proxy_base), which is a provider, not the proxy" \
...
inherited_api_base="${MOCHIII_API_BASE:-}"
export MOCHIII_API_BASE="$proxy_base"
...
if [[ -n "$inherited_api_base" && "$inherited_api_base" != "$proxy_base" ]]; then
	printf 'run-proxy: note: MOCHIII_API_BASE=%s in your environment is the direct-mode setting and is not used here; set MOCHIII_PROXY_BASE to point at a different proxy\n' \
		"$inherited_api_base" >&2
fi

# The mode switch itself. The daemon reads this and takes its key from
# MOCHIII_PROXY_KEY instead of MOCHIII_API_KEY.
export MOCHIII_USE_PROXY=true
...
if [[ -n "${MOCHIII_API_KEY:-}" ]]; then
	printf 'run-proxy: unsetting MOCHIII_API_KEY (proxy mode authenticates with the Mochiii key; the proxy holds its own OpenRouter key)\n' >&2
	unset MOCHIII_API_KEY
fi

# ---- preflight ------------------------------------------------------------

if [[ -z "${MOCHIII_PROXY_KEY:-}" ]]; then
	die "MOCHIII_PROXY_KEY is empty — the proxy authenticates every request with it" \
...
if [[ "$MOCHIII_PROXY_KEY" == sk-or-* || "$MOCHIII_PROXY_KEY" == sk-* ]]; then
	die "MOCHIII_PROXY_KEY looks like an OpenRouter key (starts with sk-), not a Mochiii key" \
...
# which validates MOCHIII_API_BASE structurally at startup but treats
# "can I reach it right now" as a runtime state rather than a startup error.
if command -v curl >/dev/null 2>&1; then
	health_url="${MOCHIII_API_BASE%/v1}/health"
	if ! health_status="$(curl -fsS -m 10 -o /dev/null -w '%{http_code}' "$health_url" 2>/dev/null)"; then
		...
		warn "if you meant to use a local proxy: MOCHIII_PROXY_BASE=http://localhost:8080/v1 $0"
...
printf 'run-proxy: proxy mode -> %s (Mochiii key set, %d chars)\n' \
	"$MOCHIII_API_BASE" "${#MOCHIII_PROXY_KEY}" >&2
