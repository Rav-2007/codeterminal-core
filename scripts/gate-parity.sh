...
reach.sh|manual|Deliberately NOT a gate yet, and the reason is its own subject matter. It reports 10 real findings on this branch today -- two branches with no upstream, and seven branches including main tracking the fork rather than the remote CI watches -- so wiring it into `make check` now would make every local run red and teach people to skip the gate, which is why fuzz.sh is excluded too. In CI it would be worse than useless: a runner's clone has no local branches, so checks 1 and 2 would examine nothing and pass by being irrelevant, exactly as hookcheck would. THE TENSION IS REAL AND IS NOT HIDDEN: a gate nobody runs is the delivery gap applied to the delivery-gap gate. Promote to `local` once those 10 findings are resolved or allowlisted with reasons -- that is the whole condition, and it is a day's work, not a design change.
